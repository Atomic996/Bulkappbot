...
export function computeIndicators(data: PriceData[]) {
  ...
  // ADX needs at least 28 candles to be reliable
  const adx = data.length >= 28
    ? ADX.calculate({ period: 14, high: highs, low: lows, close: closes })
    : [];
...
export type MarketRegime = 'TRENDING' | 'RANGING' | 'BREAKOUT' | 'UNCERTAIN' | 'LOW_VOL';
...
export function calculateTechnicalScore(indicators: any, lastPrice: number): number {
  ...
  switch (regime) {
    case 'LOW_VOL':
      return { score: 50, strategy: 'Low Volatility — No Trade' };
